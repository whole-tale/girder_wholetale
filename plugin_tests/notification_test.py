...
class NotificationTestCase(base.TestCase):
    ...
    def testSingleJobNotification(self):
        ...
        from girder.plugins.worker.constants import (
            PluginSettings as WorkerPluginSettings,
        )
    ...
    def testChainedJobNotification(self):
        ...
        self.assertNotification(
            notification["_id"], ProgressState.ACTIVE, total, 1, msg
        )
        ...
        self.assertNotification(
            notification["_id"], ProgressState.ACTIVE, total, 2, msg
        )
        ...
        self.assertEqual(job["status"], JobStatus.INACTIVE)
        self.assertNotification(
            notification["_id"], ProgressState.QUEUED, total, 2, msg
        )
        ...
        self.assertEqual(job["status"], JobStatus.SUCCESS)
        self.assertNotification(
            notification["_id"], ProgressState.ACTIVE, total, 4, msg
        )
        ...
        self.assertEqual(job["status"], JobStatus.INACTIVE)
        self.assertNotification(
            notification["_id"], ProgressState.QUEUED, total, 4, msg
        )
        ...
        self.assertEqual(job["status"], JobStatus.SUCCESS)
        self.assertNotification(
            notification["_id"], ProgressState.SUCCESS, total, 5, msg
        )
