...
class TaleTestCase(base.TestCase):
    ...
    def testTaleFlow(self):
        ...
        resp = self.request(
            ...
            type="application/json",
            body=json.dumps(
                {"imageId": str(self.image["_id"]), "dataSet": [], "public": False}
            ),
        )
        self.assertStatusOk(resp)
        # admin_tale = resp.json

        resp = self.request(path="/tale", method="GET", user=self.admin, params={})
        self.assertStatusOk(resp)
        ...
        self.assertEqual(
            set([_["_id"] for _ in resp.json]), {tale["_id"], new_tale["_id"]}
        )
        ...
        resp = self.request(
            path="/tale", method="GET", user=self.user, params={"text": "new"}
        )
        self.assertStatusOk(resp)
        self.assertEqual(len(resp.json), 1)
        self.assertEqual(set([_["_id"] for _ in resp.json]), {tale["_id"]})

        resp = self.request(
            path="/tale/{_id}".format(**new_tale), method="DELETE", user=self.admin
        )
        self.assertStatusOk(resp)

        resp = self.request(
            path="/tale/{_id}".format(**new_tale), method="GET", user=self.user
        )
        self.assertStatus(resp, 400)

        resp = self.request(
            path="/tale/{_id}".format(**tale), method="GET", user=self.user
        )
        self.assertStatusOk(resp)
    ...
    def testTaleAccess(self):
        with httmock.HTTMock(mockOtherRequest):
            ...
            resp = self.request(
                ...
                type="application/json",
                body=json.dumps(
                    {"imageId": str(self.image["_id"]), "dataSet": [], "public": True}
                ),
            )
            self.assertStatusOk(resp)
        ...
        for key in ("workspaceId",):
            resp = self.request(
                path="/folder/%s/access" % tale[key], method="GET", user=self.user
            )
            self.assertStatusOk(resp)
        ...
        resp = self.request(
            path="/image/%s/access" % tale_admin_image["imageId"],
            method="GET",
            user=self.user,
        )
        ...
        self.assertStatusOk(resp)
        resp = self.request(
            path="/tale/%s" % tale_user_image["_id"], method="GET", user=self.user
        )
        self.assertStatusOk(resp)
    ...
    def testTaleUpdate(self):
        ...
        self.assertEqual(
            resp.json["publishInfo"][0]["date"], "2019-01-23T15:48:17.476000+00:00"
        )
    ...
    def testManifest(self):
        ...
        with mock.patch(
            ...
            resp = self.request(path=pth, method="GET", user=self.user)
    ...
    def testImageBuild(self, it):
        ...
        with mock.patch(
            ...
            resp = self.request(
                path="/tale/{}/build".format(tale["_id"]), method="PUT", user=self.user
            )
            self.assertStatusOk(resp)
            job_call = mock_apply_async.call_args_list[-1][-1]
            self.assertEqual(job_call["args"], (str(tale["_id"]), False))
            self.assertEqual(
                job_call["headers"]["girder_job_title"], "Build Tale Image"
            )
        self.assertStatusOk(resp)
    ...
    def testTaleNotifications(self):
        ...
        self.assertEqual(
            events[0]["data"]["affectedResourceIds"]["taleId"], tale["_id"]
        )
        ...
        self.assertEqual(
            events[-1]["data"]["affectedResourceIds"]["taleId"], tale["_id"]
        )
        ...
        since = datetime.utcnow().isoformat()
        resp = self.request(
            path="/tale/{_id}".format(**tale), method="DELETE", user=self.admin
        )
        self.assertStatusOk(resp)
...
class TaleWithWorkspaceTestCase(base.TestCase):
    ...
    def testTaleCopy(self):
        ...
        tale = Tale().createTale(self.image, [], creator=self.admin, public=True)
        workspace = self.model("folder").load(tale["workspaceId"], force=True)
        ...
        # Create a copy
        resp = self.request(
            path="/tale/{_id}/copy".format(**tale), method="POST", user=self.user
        )
        self.assertStatusOk(resp)
    ...
    def testExportBag(self):
        ...
        bag_file = os.path.join(
            dirpath, resp.headers["Content-Disposition"].split('"')[1]
        )
    ...
    def testExportBagWithRun(self, mock_builder):
        ...
        self.assertTrue(
            os.path.exists(
                os.path.join(temp_path, "data/runs/run1/wt_quickstart.ipynb")
            )
        )
        ...
        with open(os.path.join(temp_path, "metadata/manifest.json"), "r") as f:
            ...
            items = [
                i
                for i in m["aggregates"]
                if i["uri"] == "./runs/run1/wt_quickstart.ipynb"
            ]
