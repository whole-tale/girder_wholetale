...
class Dataset(Resource):
    ...
    @autoDescribeRoute(
        Description(
            (
                "Returns all registered datasets from the system "
                "that user has access to"
            )
        )
        .param(
            "myData",
            "If True, filters results to datasets registered by the user."
            "Defaults to False.",
            required=False,
            dataType="boolean",
            default=False,
        ...
        .pagingParams(defaultSort="lowerName", defaultSortDir=SortDir.ASCENDING)
    )
    ...
    @access.user
    @autoDescribeRoute(
        Description("Remove user's reference to a registered dataset").param(
    ...
    @autoDescribeRoute(
        ...
        .notes(
            "This does not upload or copy the existing data, it just creates "
        ...
        .param(
            "parentId", "Parent ID for the new parent of this folder.", required=False
        )
        .param(
            "parentType",
        ...
        .jsonParam(
            ...
            paramType="body",
            schema=dataMapListSchema,
        )
        .param(
            "base_url",
            "The node endpoint url. This can be used "
    ...
    def importData(self, parentId, parentType, public, dataMap, base_url, params):
        user = self.getCurrentUser()
        ...
        notification = init_progress(
            resource, user, "Registering Data", "Initialization", 2
        )

        job = self._createImportJob(
            dataMap, parent, parentType, user, base_url, notification
        )
        Job().scheduleJob(job)
        return job
    ...
    @autoDescribeRoute(
        ...
        .notes(
            "Files that exist in the bag are imported directly, whereas references are only "
    ...
    def importBDBag(self, parentId, parentType, public, params):
        ...
        if cherrypy.request.headers.get("Content-Type") == "application/zip":
            return self._importBDBagFromStream(
                iterBody, parent, parentType, public, user
            )
        else:
    ...
    def _importBDBagFromStream(self, stream, parent, parentType, public, user=None):
        ...
        try:
            ...
            notification = init_progress(
                resource, user, "Importing BDBag", "Initialization", 2
            )
    ...
    def _createImportJob(
        self, data_maps, parent, parentType, user, base_url, notification
    ):
        job = Job().createLocalJob(
            title="Registering Data",
            user=user,
            type="wholetale.register_data",
            public=False,
            _async=False,
