...
class Manifest:
    ...
    def _fileList(self, doc, user=None, path="", subpath=True):
        ...
        for subfolder in Folder().childFolders(
            parentType="folder", parent=doc, user=self.user
        ):
            for item_path, item in self._fileList(
                subfolder, user=user, path=path, subpath=True
            ):
    ...
    def add_tale_records(self):
        ...
        data_dir = Folder().findOne(
            {"parentId": root_data_dir["_id"], "name": data_dir_name}
        )

        dataset_top_identifier = set()
        for girder_path, item in self._fileList(
            data_dir, user=self.user, path="/", subpath=False
        ):
            file_obj = list(Item().childFiles(item))[0]
            if "linkUrl" in file_obj:
                dataset_top_identifier.add(item["meta"]["identifier"])
                agg = {
                    "bundledAs": self.create_bundle(
                        os.path.dirname(girder_path), item["name"]
                    ),
                ...
                agg = {
                    "uri": quote("./data" + girder_path),
                    "wt:size": file_obj["size"],
                    "wt:sha512": file_obj["sha512"],
                    "wt:mimeType": file_obj["mimeType"],
                    "wt:identifier": str(file_obj["_id"]),
                }
